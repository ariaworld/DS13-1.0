...
/obj/item/organ/internal/heart/proc/handle_pulse()
	...
	if(pulse == PULSE_NONE)
		...
		should_stop = should_stop || prob(max(0, owner.getBrainLoss() - owner.max_health * 0.45)) //brain failing to work heart properly
		...
		if(should_stop) // The heart has stopped due to going into traumatic or cardiovascular shock.
			...
			pulse = PULSE_NONE
			SEND_SIGNAL(owner, COMSIG_CARBON_HEART_STOPPED)
			return
